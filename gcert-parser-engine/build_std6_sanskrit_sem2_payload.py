#!/usr/bin/env python3
"""
Builds complete std6_sanskrit_sem2_payload.json for GCERT Standard 6 Sanskrit Sem 2 (સંસ્કૃત ધોરણ 6 દ્વિતીય સત્ર).
...
pdf_url = "https://firebasestorage.googleapis.com/v0/b/quizapp-1627022258976.appspot.com/o/textbooks%2FStd%206%20Sanskrit_2Nd%20SEM.pdf?alt=media"
gs_url = "gs://quizapp-1627022258976.appspot.com/textbooks/Std 6 Sanskrit_2Nd SEM.pdf"
storage_path = "textbooks/Std 6 Sanskrit_2Nd SEM.pdf"

subject_id = "sub_sanskrit_std6_sem2"
subject_id_alt1 = "sub_sanskrut_std6_sem2"
...
def main():
    ...
    chapters_file = PROJECT_ROOT / "scratch" / "sanskrit_sem2_chapters.json"
    content_file = PROJECT_ROOT / "scratch" / "sanskrit_sem2_content.json"
        ...
        "subjects": [
            {
                ...
                "session": session,
                "name": "Sanskrit Sem 2",
                "nameGu": "સંસ્કૃત (દ્વિતીય સત્ર) ધોરણ 6",
                "name_en": "Standard 6 Sanskrit (Sem 2)",
                "name_gu": "સંસ્કૃત (દ્વિતીય સત્ર) ધોરણ 6",
                "title": "સંસ્કૃત (દ્વિતીય સત્ર) ધોરણ 6",
                "titleGu": "સંસ્કૃત (દ્વિતીય સત્ર) ધોરણ 6",
                "title_gu": "સંસ્કૃત (દ્વિતીય સત્ર) ધોરણ 6",
                "icon": "🕉️",
                "order": 4,
        ...
        "textbooks": [
            {
                ...
                "title_gu": "સંસ્કૃત (દ્વિતીય સત્ર) ધોરણ 6 પાઠ્યપુસ્તક",
                "titleGu": "સંસ્કૃત (દ્વિતીય સત્ર) ધોરણ 6 પાઠ્યપુસ્તક",
                "title_en": "Standard 6 Sanskrit (Sem 2) Textbook",
                "titleEn": "Standard 6 Sanskrit (Sem 2) Textbook",
                "subject_id": subject_id,
                "subjectId": subject_id,
                "standard_id": "6",
                "standardId": "6",
    ...
    for ch_info in raw_chapters_info:
        ...
        ch_id = f"sub_sanskrit_std6_sem2_ch{c_num}"
        ...
        if q_list:
            quiz_id = f"quiz_std6_sanskrit_sem2_{ch_id}"
            question_docs = []
            question_ids = []
            ...
            for q_idx, q in enumerate(q_list):
                ...
                qz_q_id = f"q_std6_sanskrit_sem2_{ch_id}_{q_idx+1}"
        ...
        if fc_list:
            for idx, fc in enumerate(fc_list):
                ...
                fc_id = f"fc_std6_sanskrit_sem2_{ch_id}_{idx+1}"
        ...
        content = f"વિષય: સંસ્કૃત (દ્વિતીય સત્ર) ધોરણ 6\nપ્રકરણ {c_num}: {title_gu} ({title_en})\nવર્ણન: {desc_gu}\n"
        if q_summary:
            content += f"\nમુખ્ય પ્રશ્નોત્તરી:\n{q_summary}\n"
        if fc_summary:
        ...
        payload["ai_knowledge_base"].append({
            "kb_id": f"kb_std6_sanskrit_sem2_{ch_id}",
            "standard_id": "6",
            "standard_number": standard_number,
            "session": session,
            "subject_id": subject_id,
            ...
            "title_gu": title_gu,
            "content_gu": content,
            "keywords": [title_gu, title_en, "સંસ્કૃત", "ધોરણ 6", "sanskrit", "દ્વિતીય સત્ર", "sem 2"],
    ...
    output_file = PROJECT_ROOT / "outputs" / "std6_sanskrit_sem2_payload.json"
    os.makedirs(output_file.parent, exist_ok=True)
    with open(output_file, "w", encoding="utf-8") as f:
        json.dump(payload, f, ensure_ascii=False, indent=2)

    print(f"✅ Generated Standard 6 Sanskrit Sem 2 payload: {output_file}")
