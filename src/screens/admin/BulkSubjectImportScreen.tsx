/**
 * BulkSubjectImportScreen.tsx
 *
 * Six-phase bulk import for Subjects:
 *   Phase 1 — Schema verification (fetch real Firestore doc)
 *   Phase 2 — Uses createSubject() from content.service.ts (same as Add Subject)
 ...
 *
 * ADDITIVE ONLY — does NOT touch ContentManagementScreen or any existing code.
 */
...
import {
  validateSubjectRows,
  importSubjects,
  validateSchemaAgainstFirestore,
  SubjectImportRow,
  SubjectImportPreview,
  SubjectCreatePayload,
  SUBJECT_CSV_TEMPLATE_HEADERS,
  SUBJECT_CSV_TEMPLATE_ROWS,
} from '../../services/bulkImport/subjectImport.service';
import { saveImportLog, fetchImportLogs, ImportLog } from '../../services/bulkImport/importLog.service';
import { getTotalSubjects } from '../../services/firebase/content.service';

// ─── Step Types ───────────────────────────────────────────────────────────────

type Step =
  | 'upload'           // initial upload screen
  | 'schema_check'     // Phase 4: comparing schema
  | 'validating'       // Phase 1: validating rows
  | 'dry_run'          // Phase 5: dry run preview
  | 'payload_preview'  // Phase 3: payload detail view
  | 'importing'        // actual write
  | 'report'           // Phase 6: post-import verification
  | 'history';

// ─── Screen ───────────────────────────────────────────────────────────────────

export function BulkSubjectImportScreen(): React.JSX.Element | null {
  const [step, setStep] = useState<Step>('upload');
  const [fileName, setFileName] = useState('');
  const [rawRows, setRawRows] = useState<Record<string, string>[]>([]);
  const [preview, setPreview] = useState<SubjectImportPreview | null>(null);
  ...
  const [selectedPayload, setSelectedPayload] = useState<SubjectCreatePayload | null>(null);
  ...
  const handleDownloadTemplate = useCallback(async () => {
    try {
      const csv = generateCSV(SUBJECT_CSV_TEMPLATE_HEADERS, SUBJECT_CSV_TEMPLATE_ROWS);
      await Share.share({ message: csv, title: 'subject_import_template.csv' });
    } catch (err) {
  ...
  const handlePickFile = useCallback(async () => {
    try {
      ...
      const required = ['standardid', 'session', 'name', 'namegu', 'order'];
      ...
      const schemaResult = await validateSchemaAgainstFirestore();
      ...
      setStep('validating');
      const result = await validateSubjectRows(rows, (checked, total) => {
        setProgress(checked);
  ...
  const handleConfirmImport = useCallback(async () => {
    ...
    // Count before import (Phase 6)
    const countBefore = await getTotalSubjects();
    ...
    try {
      const result = await importSubjects(preview.validRows, (done, total) => {
        setProgress(done);
      ...
      // Count after import (Phase 6)
      const countAfter = await getTotalSubjects();

      // Save import log (non-critical — failure must NOT block the report screen)
  ...
  const loadHistory = useCallback(async () => {
    setLogsLoading(true);
    try {
      const data = await fetchImportLogs('subject');
      setLogs(data);
    } catch (_err) {
  ...
  // ─── Payload Preview Modal (Phase 3) ──────────────────────────────────────

  const openPayloadPreview = useCallback((payload: SubjectCreatePayload) => {
  ...
  const allPreviewRows: SubjectImportRow[] = preview
    ...
    return (
      <ScrollView style={styles.container} contentContainerStyle={styles.scrollContent}>
        <View style={styles.heroCard}>
          ...
          <Text style={styles.heroTitle}>Bulk Subject Import</Text>
          <Text style={styles.heroSubtitle}>
            Import hundreds of subjects. Uses the exact same logic as Add Subject.
          </Text>
        </View>
        ...
        <View style={styles.phasesCard}>
          ...
          {[
            { n: '4', label: 'Schema Validation', desc: 'Compare against real Firestore doc' },
            { n: '1', label: 'Row Validation', desc: 'Validate required fields + duplicates' },
            { n: '5', label: 'Dry Run Preview', desc: 'Review before any write' },
            { n: '3', label: 'Payload Preview', desc: 'See exact data that will be saved' },
            { n: '2', label: 'Import via createSubject()', desc: 'Same function as Add Subject screen' },
            { n: '6', label: 'Post-Import Verify', desc: 'Confirm count increased correctly' },
          ].map(p => (
        ...
        <View style={styles.schemaCard}>
          ...
          <Text style={styles.schemaNote}>
            ✅ Uses{' '}
            <Text style={styles.code}>createSubject()</Text>
            {' '}from content.service.ts — identical to Add Subject
          </Text>
          <View style={styles.columnsList}>
            {['standardId', 'session', 'name', 'nameGu', 'icon', 'order',
        ...
        <TouchableOpacity style={styles.historyBtn} onPress={() => { setStep('history'); loadHistory(); }}>
  ...
  if (step === 'schema_check') {
    return (
      <View style={styles.centered}>
        ...
        <Text style={styles.validatingSubtitle}>Comparing against existing Firestore document</Text>
      </View>
    );
  }
      ...
      <View style={styles.container}>
        ...
        <View style={styles.dryRunBanner}>
          ...
          <View style={styles.dryRunInfo}>
            <Text style={styles.dryRunTitle}>Phase 5: Dry Run — No data saved yet</Text>
        ...
        {schemaDoc && (
          <View style={styles.schemaCompareBar}>
            <Text style={styles.schemaCompareText}>
              ✅ Schema matches existing Firestore structure ({Object.keys(schemaDoc).length} fields verified)
            </Text>
          </View>
        )}
        ...
        {parseErrors.length > 0 && (
          <View style={styles.parseErrorBanner}>
            <Text style={styles.parseErrorText}>⚠️ {parseErrors.length} parse warning(s) — some rows were skipped</Text>
          </View>
        )}
            ...
            <View style={styles.previewRow}>
              <View style={styles.previewRowLeft}>
                ...
                <View style={styles.previewRowInfo}>
                  <Text style={styles.previewRowName} numberOfLines={1}>{item.payload.name}</Text>
                  <Text style={styles.previewRowMeta}>
                    Std {item.payload.standardId} • Sem {item.payload.session} • {item.payload.icon} • Order {item.payload.order}
                  </Text>
                  {item.reason ? (
              ...
              <View style={styles.previewRowRight}>
                ...
                {item.status === 'valid' && (
                  <TouchableOpacity
                    style={styles.previewPayloadBtn}
                    onPress={() => openPayloadPreview(item.payload)}
                  >
                    <Text style={styles.previewPayloadBtnText}>👁 Payload</Text>
                  </TouchableOpacity>
                )}
              </View>
            </View>
        ...
        <View style={styles.actionBar}>
          ...
          <TouchableOpacity
            style={[styles.importBtn, preview.validRows.length === 0 && styles.importBtnDisabled]}
            onPress={handleConfirmImport}
            disabled={preview.validRows.length === 0}
          >
            <Text style={styles.importBtnText}>
              ✅ Import {preview.validRows.length} Subject{preview.validRows.length !== 1 ? 's' : ''}
            </Text>
          </TouchableOpacity>
        </View>
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.modalCard}>
              ...
              <Text style={styles.modalSubtitle}>
                This exact object will be passed to createSubject()
              </Text>
              {selectedPayload && (
                <ScrollView style={styles.payloadScroll}>
                  {Object.entries(selectedPayload).map(([k, v]) => (
                    <View key={k} style={styles.payloadRow}>
                      ...
                      <Text style={styles.payloadValue}>{String(v ?? '')}</Text>
                    </View>
                  ))}
                  <View style={styles.payloadDivider} />
                  <Text style={styles.payloadNote}>Added by createSubject():</Text>
                  {['isDeleted: false', 'createdAt: serverTimestamp()', 'updatedAt: serverTimestamp()'].map(f => (
                    <Text key={f} style={styles.payloadAutoField}>  {f}</Text>
                  ))}
                </ScrollView>
              )}
              <TouchableOpacity
    ...
    return (
      <View style={styles.centered}>
        <View style={styles.importingCard}>
          ...
          <Text style={styles.importingNote2}>Calling createSubject() for each row</Text>
          <Text style={styles.importingSubtitle}>{progress} / {progressTotal} subjects written</Text>
          <View style={styles.progressTrack}>
            <Animated.View style={[styles.progressBar, { width: `${Math.round(pct * 100)}%` }]} />
          </View>
      ...
      <ScrollView style={styles.container} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={[styles.verifyCard, { borderColor: countIncreased ? adminColors.success : adminColors.warning }]}>
          ...
          <View style={styles.verifyRow}>
            <Text style={styles.verifyLabel}>Subject count before:</Text>
            <Text style={styles.verifyValue}>{importResult.countBefore}</Text>
          </View>
          <View style={styles.verifyRow}>
            <Text style={styles.verifyLabel}>Subject count after:</Text>
          ...
          <View style={styles.verifyRow}>
            ...
            <Text style={[styles.verifyValue, {
              color: countIncreased ? adminColors.success : adminColors.warning,
            }]}>
              +{importResult.countAfter - importResult.countBefore}
            </Text>
          </View>
          {countIncreased ? (
            <Text style={styles.verifyPass}>✅ Subject listing will show new subjects immediately</Text>
          ) : (
        ...
        <TouchableOpacity style={styles.importBtn} onPress={handleReset}>
            ...
            renderItem={({ item }) => (
              <View style={styles.logCard}>
                <View style={styles.logCardHeader}>
                  ...
                  <View style={[styles.badge, { backgroundColor: adminColors.primary + '25' }]}>
                    <Text style={[styles.badgeText, { color: adminColors.primary }]}>subject</Text>
                  </View>
                </View>
...
const styles = StyleSheet.create({
  ...
  phaseNumText: { fontSize: typography.size.sm, fontWeight: typography.weight.bold, color: adminColors.textInverse },
  phaseInfo: { flex: 1 },
  ...
  schemaNote: { fontSize: typography.size.sm, color: adminColors.textSecondary, marginBottom: spacing.md },
  code: { fontFamily: 'monospace', backgroundColor: adminColors.surfaceElevated, color: adminColors.accent },
  ...
  columnChipText: { fontSize: typography.size.xs, color: adminColors.accent, fontWeight: typography.weight.semibold },
  ...
  uploadZone: {
    borderWidth: 2, borderStyle: 'dashed', borderColor: adminColors.primary,
    borderRadius: borderRadius.xl, padding: spacing.xxxl, alignItems: 'center',
    marginBottom: spacing.xl, backgroundColor: adminColors.primary + '08',
  },
  uploadZoneIcon: { fontSize: 40, marginBottom: spacing.md },
  uploadZoneTitle: { fontSize: typography.size.lg, fontWeight: typography.weight.bold, color: adminColors.primary },
  ...
  dryRunBanner: {
    ...
    backgroundColor: adminColors.info + '20', margin: spacing.lg,
    borderRadius: borderRadius.md, padding: spacing.md,
    borderWidth: 1, borderColor: adminColors.info,
  },
  dryRunIcon: { fontSize: 24, marginRight: spacing.md },
  dryRunInfo: { flex: 1 },
  dryRunTitle: { fontSize: typography.size.sm, fontWeight: typography.weight.bold, color: adminColors.info },
  ...
  changeFileText: { fontSize: typography.size.sm, color: adminColors.primary, fontWeight: typography.weight.semibold },
  ...
  previewPayloadBtn: {
    backgroundColor: adminColors.info + '20', paddingHorizontal: spacing.sm,
    paddingVertical: 2, borderRadius: borderRadius.xs, marginTop: spacing.xs,
  },
  previewPayloadBtnText: { fontSize: typography.size.xs, color: adminColors.info },
  ...
  importBtn: { flex: 2, backgroundColor: adminColors.primary, borderRadius: borderRadius.md, paddingVertical: spacing.md, alignItems: 'center' },
  importBtnDisabled: { opacity: 0.4 },
  importBtnText: { fontSize: typography.size.md, color: adminColors.textInverse, fontWeight: typography.weight.bold },
  ...
  payloadKey: { fontSize: typography.size.sm, color: adminColors.accent, fontWeight: typography.weight.semibold, flex: 1 },
  ...
  payloadAutoField: { fontSize: typography.size.xs, color: adminColors.accentGreen, fontFamily: 'monospace' },
  modalCloseBtn: {
    backgroundColor: adminColors.primary, borderRadius: borderRadius.md,
    paddingVertical: spacing.md, alignItems: 'center', marginTop: spacing.lg,
  },
  modalCloseBtnText: { fontSize: typography.size.md, fontWeight: typography.weight.bold, color: adminColors.textInverse },
  ...
  importingNote2: { fontSize: typography.size.xs, color: adminColors.accent, marginBottom: spacing.md },
  ...
  progressBar: { height: 8, backgroundColor: adminColors.primary, borderRadius: 4 },
  progressPct: { fontSize: typography.size.lg, fontWeight: typography.weight.bold, color: adminColors.primary },
  ...
  backBtnText: { fontSize: typography.size.md, color: adminColors.primary, fontWeight: typography.weight.semibold, marginRight: spacing.md },
