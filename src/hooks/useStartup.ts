...
import {
  ...
  MobileAppConfig,
  VersionStatus,
} from '../services/firebase/version.check.service';
...
export function useStartup(): StartupState {
  ...
  const [updateConfig, setUpdateConfig] = useState<MobileAppConfig | null>(
    null,
  );
  ...
  const runStartupChecks = useCallback(async () => {
    ...
    let connected = false;
    try {
      const state = await withTimeout(NetInfo.fetch(), 3000);
      connected =
        state.isConnected === true && state.isInternetReachable !== false;
    } catch (err) {
      console.warn('[useStartup] Network check failed or timed out:', err);
      connected = false;
    }
    ...
    try {
      // checkAppVersion might hang if Firestore is unreachable/offline, so wrap in a 4s timeout
      const result = await withTimeout(checkAppVersion(APP_VERSION), 4000, {
        status: 'up_to_date' as VersionStatus,
        config: null,
      });
      versionStatus = result.status;
      config = result.config;
      console.log(
        '[useStartup] Version check:',
        versionStatus,
        'current:',
        APP_VERSION,
      );
    ...
    // ── 3. All clear ──────────────────────────────────────
    console.log('[useStartup] All checks passed — setting phase to ready');
    setPhase('ready');
  }, []);
